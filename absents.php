...
if ( isset($_SESSION['idUser']) && isset($_SESSION['userEmail'])) { 
    ...
    $etudiants = $cnx->prepare("SELECT * from Etudiant e join absence a on e.idEtudiant = a.idEtudiant WHERE a.idCour=? AND a.status = 'nonJustifiée' order by prenom");
    $etudiants->execute([$_GET['id']]);
...
<div class="row"><div class="col-6"><h1 class="pb-2 display-4">Liste des absences</h1></div></div>
...
<div class="modal fade" id="groupeModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Liste des Justifications</h5>
  ...
  <thead>
    <tr>
      ...
      <th scope="col" class="text-end">Absence Justifiée</th>
    </tr>
  </thead>
  <tbody>
...
<?php 
 if (isset($_POST["terminer"]) && isset($_POST["absentId"])) {
  foreach ($_POST["absentId"] as $absentId) {
    $absent = $cnx->prepare("UPDATE absence SET status='justifiée' WHERE idEtudiant = ? AND idCour = ? ") ;
    $absent->execute([ $absentId,$_GET['id']]);
  }
  echo "<script>";
  echo "document.location.replace('justifications.php')"; // redirection 
echo "</script>";
}
    }
}else{
